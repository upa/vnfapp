...
int vale_rings = 0;
int verbose;
patricia_tree_t * tree;		/* routing table */
...
int
split_prefixdst (char * str, void * prefix, int * len, char * out)
{
	int n, l, c = 0;
	char * p, * args[3];
	...
	*out = *args[2];
	...
	inet_pton (AF_INET, args[0], prefix);
...
move (struct vnfapp * va)
{
	...
	while (burst-- > 0) {
		...
		/* route entry check */
		route = find_patricia_entry (tree, &ip->ip_dst, 32);
		if (!route) {
			goto drop;
		}
		if (!(IS_L2R (v) && route->out == 'r') &&
		    !(IS_R2L (v) && route->out == 'l')) {
			goto drop;
		}
...
main (int argc, char ** argv)
{
	...
	while ((ch = getopt (argc, argv, "r:l:q:R:L:f:e:")) != -1) {
		switch (ch) {
		case 'r' :
			rif = optarg;
			break;
		...
		case 'f' :
			...
			D ("install Route Entry %s", optarg);

			ret = split_prefixdst (optarg, &prefix, &len, &out);
			...
			/* install route  */
			route = (struct routeentry *)
				malloc (sizeof (struct routeentry));
			memset (route, 0, sizeof (struct routeentry));
			route->out = out;

			add_patricia_entry (tree, &prefix, len, route);
